<template>
  <article class="bg-white rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all group">
    <NuxtLink :to="`/articles/${article.id}`" class="flex flex-col h-80 p-6">
      <!-- Article Meta -->
      ...
      <!-- Article Excerpt -->
      <p class="text-gray-600 text-sm mb-4 line-clamp-3 flex-grow" data-testid="article-excerpt">
        {{ article.excerpt }}
      </p>

      <!-- Tags -->
      <div v-if="article.tags && article.tags.length > 0" class="flex flex-wrap gap-1 mb-4 flex-shrink-0" data-testid="article-tags">
        <span
